"""
東証プライム全銘柄 10年分価格データ取得
- v1 get_prices_daily_quotes 相当機能をv2 get_eq_bars_dailyで実現
- 期間：2016-04-25 ～ 2026-04-24（Standardプランカバー範囲）
- 対象：東証プライム全銘柄（フィルタなし）
- 保存：data/raw/prices_10y.parquet
"""
import os
import sys
import time
...
def main() -> None:
    ...
    print(f"  手法: 銘柄ごと逐次取得（{SLEEP_SEC}秒間隔・120req/min準拠）")
    print(f"  出力: {OUTPUT}")
    print("=" * 64)
    print()
    print("  ※ v1 get_prices_daily_quotes はrefresh_token認証非対応のため")
    print("     v2 get_eq_bars_daily（同等機能）で取得します")
    ...
    if failed:
        print(f"  取得失敗      : {len(failed)} 銘柄")
    print(f"  所要時間      : {elapsed_total:.0f}秒")
    print("=" * 64)
    print()
    print("  既存ファイル data/raw/prices_all_prime.parquet は保持済み")
