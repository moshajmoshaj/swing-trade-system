...
from strategy_c import generate_signals
from utils.risk import calc_position_size

load_dotenv()

INITIAL_CAPITAL = 1_000_000
ATR_STOP_MULT   = 1.5  # 戦略C: 損切り ATR × 1.5
ATR_TP_MULT     = 1.5  # 戦略C: 利確 ATR × 1.5（簡略化版・SMA20回帰は将来実装）
MAX_HOLD_DAYS   = 7    # 戦略C: 最大保有7日
PRIME_CACHE     = Path("data/raw/prices_10y.parquet")
CANDIDATE_CSV   = Path("logs/strategy_c_candidates.csv")
...
def save_candidate_chart(top_results: dict[str, dict], names: dict[str, str]) -> None:
    path = "logs/strategy_c_equity.png"
    ...
    ax.set_title("戦略C 運用候補銘柄 損益曲線（上位20銘柄）", fontsize=14)
    ax.set_xlabel("日付")
    ax.set_ylabel("資産（円）")
